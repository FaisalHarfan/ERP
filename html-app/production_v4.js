...
function prodStatusColor(status) {
    return { DRAFT: 'bg-gray-100 text-gray-600', IN_PROGRESS: 'bg-blue-100 text-blue-700', PARTIAL: 'bg-indigo-100 text-indigo-700', DONE: 'bg-green-100 text-green-700' }[status] || 'bg-gray-100 text-gray-600';
}
...
window.renderProductionDashboard = () => {
    ...
    const dashboardCards = PROD_STAGES.map((st, idx) => {
        const stageMOs = filteredMOs.filter(m => {
...
// --- DAFTAR MO ---
window.renderProductionMO = async () => {
    const canEdit = getModulePermission('produksi').edit || isCurrentUserAdmin();
    ...
    if (filtered.length === 0) {
        ...
        filtered.forEach(mo => {
            ...
            <tr class="border-b border-slate-100 hover:bg-slate-50 transition-all text-sm">
                ...
                <td class="py-4 px-4"><span class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${prodStatusColor(mo.status)}">${mo.status === 'DONE' ? 'Selesai' : (mo.status === 'PARTIAL' ? 'Sebagian' : 'Proses')}</span></td>
                <td class="py-4 px-5 text-right whitespace-nowrap overflow-visible">
                    <div class="flex justify-end overflow-visible">
                        <div class="relative inventory-dropdown-container">
                            <button onclick="window.toggleRowDropdown(event, '${mo.id}')" 
                            ...
                            <div id="dropdown-${mo.id}" class="inventory-action-menu hidden absolute right-0 top-full mt-1 w-44 bg-white rounded-xl shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1),0_8px_10px_-6px_rgba(0,0,0,0.1)] border border-slate-100 z-[2000] overflow-hidden text-left animate-in fade-in zoom-in-95 duration-100">
                                <div class="py-1.5 flex flex-col">
                                    ...
                                    <button onclick="deleteMO('${mo.id}')" class="text-left px-4 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50/50 flex items-center gap-3 transition-colors">
                                        <i class="fas fa-trash w-4 text-rose-400"></i> Hapus MO
                                    </button>
                                    ` : ''}
                                </div>
                            </div>
...
window.openMOModal = async (stagePreset = '') => {
    ...
    const mc = document.getElementById('main-content');
    mc.innerHTML = `<div class="flex items-center justify-center h-64"><i class="fas fa-spinner fa-spin text-3xl text-blue-500"></i><p class="ml-4 text-slate-500 font-semibold">Memuat data BOM & Item...</p></div>`;

    // Sync data wajib dari server sebelum form dibuild
    try {
        await Promise.all([
            db.sync('bomHeaders'),
            db.sync('bomMaterials'),
            db.sync('inventoryItems'),
            db.sync('machines')
        ]);
    } catch(e) { console.warn('openMOModal sync error:', e); }
                    ...
                    <div id="mo_grp_product" class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm space-y-4">
                        ...
                        <div id="mo_product_single_wrap">
                            ...
                            <div class="flex gap-2">
                                <div class="relative flex-1">
                                    ...
                                    <div id="mo_product_dropdown_single"
                                        ...
                                        <div class="p-2 border-t border-slate-100 bg-slate-50/50">
                                            <button type="button" onclick="openQuickAddProductModal('single')" class="w-full flex items-center gap-3 p-2.5 hover:bg-white hover:shadow-sm rounded-xl text-[11px] font-black text-slate-500 hover:text-indigo-600 transition-all uppercase tracking-wider">
                                ...
                                <button type="button" onclick="openQuickAddProductModal('single')"
                                    class="shrink-0 w-12 h-12 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-600 hover:text-blue-700 transition-all flex items-center justify-center shadow-sm active:scale-95"
...
window.updateMOForm = () => {
    ...
    if (stage === 'OVEN_BASAH' || stage === 'OVEN_KERING') {
        ...
        if (isPacking) {
            // PACKING (Gudang Jadi): gunakan multi-product UI tanpa mesin
            document.getElementById('mo_product_single_wrap')?.classList.add('hidden');
            document.getElementById('mo_product_multi_wrap')?.classList.remove('hidden');
            if (document.getElementById('mo_target_product_rows')?.children.length === 0) {
                setTimeout(() => { if (window.addMOTargetProductRow) addMOTargetProductRow(stage); }, 30);
...
// loadBOMMaterialsToMO — defined below (async, uses bomMaterials table)
...
window.startMO = async () => {
    ...
    const isMultiRow = isOvenBasah || stage === 'OVEN_KERING' || stage === 'PACKING';
    let productId = '';
    let productName = '';
    if (isMultiRow) {
        // Baca dari baris pertama multi-product list
    ...
    // For OVEN_BASAH, OVEN_KERING & PACKING: read target products + their assigned machines from multi-rows
    if (isMultiRow) {
        ...
        tpRows.forEach((row, i) => {
            ...
            if (qty <= 0) { tpErrors.push(`Produk baris ${i + 1}: Qty target wajib diisi dan harus lebih besar dari 0`); return; }
...
window.openCompleteMOModal = (id) => {
    ...
    if (mo.stage === 'OVEN_BASAH') {
        ...
        // Calculate cumulative raw material consumed
        const cumulativeRM = {};
        history.forEach(h => {
            (h.inputItems || []).forEach(item => {
                cumulativeRM[item.inventoryItemId] = (cumulativeRM[item.inventoryItemId] || 0) + parseFloat(item.qty || 0);
            });
        });
        
        // Calculate cumulative WIP produced
        const cumulativeWIP = {};
        history.forEach(h => {
            (h.outputProducts || []).forEach(op => {
                cumulativeWIP[op.itemId] = (cumulativeWIP[op.itemId] || 0) + parseFloat(op.qty || 0);
            });
        });

        // Build the raw material actual input rows
        const matRowsOB = (mo.inputItems || []).map(m => {
            const consumed = cumulativeRM[m.inventoryItemId] || 0;
            const remaining = Math.max(0, parseFloat(m.qty) - consumed);
            return `
            <div class="flex items-center gap-3 bg-slate-50 p-2 rounded-lg border border-slate-100">
                <div class="flex-1">
                    <div class="text-xs font-bold text-slate-700">${m.itemName}</div>
                    <div class="text-[9px] text-slate-400 mt-0.5">Resep: ${prodFmt(m.qty)} | Sudah Terpakai: ${prodFmt(consumed)} ${m.unit}</div>
                </div>
                <div class="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded border border-slate-200 shadow-sm focus-within:border-slate-400 transition-all">
                    <input type="text" inputmode="decimal" oninput="formatNumericInput(this)" class="mo_final_rm_actual w-20 bg-transparent border-0 text-sm font-black text-slate-700 text-right focus:ring-0 outline-none" data-item-id="${m.inventoryItemId}" data-item-name="${m.itemName}" data-item-unit="${m.unit}" value="${prodFmt(remaining)}">
                    <span class="text-[9px] font-black text-slate-400 uppercase tracking-tighter">${m.unit || 'Kg'}</span>
                </div>
            </div>`;
        }).join('');

        // Build pre-filled product result rows from targetProducts
        const tpList = mo.targetProducts || [];
        const resultRowsHtml = tpList.map((tp, idx) => {
            const produced = cumulativeWIP[tp.itemId] || 0;
            const remaining = Math.max(0, parseFloat(tp.qty) - produced);
            return `
            <div class="ob-output-row flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100" data-item-id="${tp.itemId}">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm font-black text-slate-800">${tp.itemName}</p>
                    <p class="text-[9px] text-slate-400 mt-0.5">Target MO: ${prodFmt(tp.qty)} Kg | Sudah Selesai: ${prodFmt(produced)} Kg</p>
                </div>
                <div class="flex flex-col items-end gap-1">
                    <label class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Qty Hasil Kali Ini (Kg)</label>
                    <div class="flex items-center gap-1.5 bg-white border-2 border-slate-200 rounded-xl px-3 py-2 focus-within:border-blue-400 transition-all">
                        <input type="text" inputmode="decimal" oninput="formatNumericInput(this)" placeholder="0"
                            class="ob-out-qty w-24 bg-transparent border-0 text-xl font-black text-slate-800 text-right focus:ring-0 outline-none" value="${prodFmt(remaining)}">
        ...
        dynamicBody = `
            <div class="space-y-2 mb-4">
                <h4 class="text-[10px] font-black text-slate-400 mb-3 uppercase tracking-[0.2em] flex items-center gap-2">
                    <i class="fas fa-clipboard-check text-blue-600"></i> Realisasi Pemakaian Bahan Oven Basah (Kali Ini)
                </h4>
                <div class="space-y-2">${matRowsOB}</div>
            </div>

            <div class="bg-white border-2 border-slate-100 p-4 rounded-xl shadow-sm space-y-4">
                <h4 class="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2 mb-3">
                    <i class="fas fa-boxes text-orange-500"></i> Produk Hasil Oven Basah (Kali Ini)
                </h4>
                <div id="ob_output_rows" class="space-y-2">${resultRowsHtml}</div>
            </div>`;

        // Render previous completion history
        if (history.length > 0) {
            const historyItems = history.map((h, i) => {
                const inps = (h.inputItems || []).map(itm => `${itm.itemName}: ${prodFmt(itm.qty)} ${itm.unit}`).join(', ');
                const outs = (h.outputProducts || []).map(op => `${op.itemName}: ${prodFmt(op.qty)} Kg`).join(', ');
                return `
                    <div class="relative pl-6 pb-4 border-l-2 border-slate-200 last:pb-0 last:border-transparent">
                        <!-- Indicator circle -->
                        <span class="absolute left-[-6px] top-1 w-2.5 h-2.5 rounded-full bg-blue-500 ring-4 ring-white"></span>
                        <div class="text-[11px] font-black text-blue-600 uppercase tracking-wider">${new Date(h.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}</div>
                        <div class="text-[10px] text-slate-400 mt-0.5 font-medium">Oleh: ${h.createdBy || '-'}</div>
                        <div class="text-xs text-slate-600 mt-1 font-semibold">Bahan Baku: <span class="text-slate-500 font-normal">${inps || '-'}</span></div>
                        <div class="text-xs text-slate-600 font-semibold">Hasil Produksi: <span class="text-green-600 font-bold">${outs || '-'}</span></div>
                        ${h.notes ? `<div class="text-[11px] bg-slate-100 text-slate-600 p-2 rounded mt-1.5 font-medium italic">"${h.notes}"</div>` : ''}
                    </div>`;
            }).join('');
            ...
            historyHtml = `
                <div class="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm">
                    <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-4">
                        <i class="fas fa-history text-blue-600"></i> Riwayat Penyelesaian Parsial (${history.length})
                    </h3>
                    <div class="space-y-1">${historyItems}</div>
                </div>`;
        }
        ...
        const tpList = mo.targetProducts || [];
        if (tpList.length === 0 && mo.productId) {
            tpList.push({ itemId: mo.productId, itemName: mo.productName, qty: mo.inputQty || 0 });
        }

        const packRows = tpList.map((tp, idx) => {
            const inputWipId = db.ensureWIPItem(tp.itemId, 'Oven Kering', false);
            const inputItem = inputWipId ? db.findById('inventoryItems', inputWipId) : null;
            const inputStock = inputWipId ? db.getInventoryStock(inputWipId, 'OVEN_KERING') : 0;
            const baseProd = db.findById('inventoryItems', tp.itemId);

            return `
            <div class="packing-result-row bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4"
                data-tp-idx="${idx}"
                data-item-id="${tp.itemId}"
                data-wip-item-id="${inputWipId || ''}"
                data-target-qty="${tp.qty || 0}">

                <!-- Header Row: Produk -->
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-0.5">Produk</p>
                        <p class="text-sm font-black text-slate-800">${baseProd?.itemName || tp.itemName || `Produk ${idx+1}`}</p>
                        <p class="text-[9px] text-slate-400 mt-0.5">${baseProd?.itemCode || ''}</p>
                    </div>
                </div>

                <!-- Inputs Row -->
                <div class="grid grid-cols-2 gap-4">
                    <!-- Qty Pemakaian Oven Kering (WIP) -->
                    <div class="bg-white border-2 border-slate-200 rounded-xl p-3 focus-within:border-blue-500 transition-all">
                        <label class="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Pengeluaran Actual (Kg) <span class="text-red-500">*</span></label>
                        <div class="flex items-center gap-1.5">
                            <input type="text"
                                class="pack-input-actual w-full bg-transparent text-xl font-black text-slate-800 text-right outline-none"
                                placeholder="0"
                                value="${prodFmt(tp.qty || 0)}"
                                oninput="formatNumericInput(this); recalcPackingSacks(${idx})"
                                data-row="${idx}">
                            <span class="text-xs font-black text-slate-400">Kg</span>
                        </div>
                        <p class="text-[8px] text-slate-400 mt-1">Stok Oven Kering: ${prodFmt(inputStock)} Kg</p>
                    </div>

                    <!-- Hasil Pengepakan Sak -->
                    <div class="bg-white border-2 border-slate-200 rounded-xl p-3 focus-within:border-blue-500 transition-all">
                        <label class="block text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Jumlah Karung (SETARA 25 KG / SAK) <span class="text-red-500">*</span></label>
                        <div class="relative flex items-center gap-1.5">
                            <input type="text"
                                class="pack-output-qty w-full bg-transparent text-xl font-black text-slate-800 text-right outline-none"
                                placeholder="0"
                                value="${Math.floor((tp.qty || 0) / 25)}"
                                oninput="formatNumericInput(this)"
                                data-row="${idx}">
                            <span class="text-[9px] font-black text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded">SACK</span>
                        </div>
                        <p class="text-[8px] text-slate-400 mt-1">Auto: Hasil bagi-25</p>
                    </div>
                </div>
                ...
                ${baseProd ? `
                <div class="flex items-center gap-2 bg-indigo-50 border border-indigo-100 rounded-xl px-3 py-2">
                    <i class="fas fa-warehouse text-indigo-400 text-xs"></i>
                    <div>
                        <span class="text-[8px] font-black text-indigo-300 uppercase tracking-widest">Stok Tujuan: </span>
                        <span class="text-xs font-black text-indigo-800">${baseProd.itemName}</span>
                    </div>
                </div>` : ''}
            </div>`;
        }).join('');
        ...
        dynamicBody = `
            <div class="space-y-4">
                <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                    <i class="fas fa-box-open text-green-500"></i> Hasil Pengepakan Sak & Gudang Jadi
                </h4>
                ${packRows || '<p class="text-sm text-slate-400 italic">Tidak ada produk terdaftar pada MO ini.</p>'}
            </div>`;
    }
                        ...
                        <div class="grid grid-cols-1 ${mo.stage === 'OVEN_BASAH' ? 'sm:grid-cols-2' : 'sm:grid-cols-3'} gap-6">
                            ...
                            <div>
                             ...
                             <div class="searchable-select-container relative">
                                 ...
                                 <input type="hidden" id="mo_partial_status" value="PARTIAL">
                                 <button type="button" onclick="toggleSearchableSelect('mo_partial_status')"
                                     class="w-full h-[52px] bg-slate-50 border-2 border-transparent rounded-2xl px-5 py-3 text-sm font-black text-slate-700 outline-none cursor-pointer flex justify-between items-center transition-all hover:bg-slate-200/50 shadow-sm">
                                     <span id="mo_partial_status_display" class="text-slate-800">Partial</span>
                                     <i class="fas fa-chevron-down text-slate-400 text-[10px]"></i>
                                 </button>
                                 <div id="mo_partial_status_dropdown" class="searchable-dropdown hidden absolute left-0 right-0 top-full mt-2 bg-white rounded-2xl shadow-xl border border-slate-100 z-[200] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                     <div class="p-3 border-b border-slate-50 bg-slate-50/50">
                                         <div class="relative">
                                             <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 text-xs"></i>
                                             <input type="text" placeholder="Cari status..." onkeyup="filterSearchableOptions('mo_partial_status')" id="mo_partial_status_search"
                                                 class="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500/20">
                                         </div>
                                     </div>
                                     <div class="max-h-60 overflow-y-auto p-1" id="mo_partial_status_options">
                                         <div onclick="window.selectMOStatus('PARTIAL', 'Partial')"
                                             class="px-4 py-2.5 rounded-lg text-sm font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-600 cursor-pointer transition-colors flex items-center justify-between">
                                             <span>Partial</span>
                                             <i class="fas fa-check text-[10px] text-blue-500 opacity-100"></i>
                                         </div>
                                         <div onclick="window.selectMOStatus('DONE', 'Finish')"
                                             class="px-4 py-2.5 rounded-lg text-sm font-bold text-slate-600 hover:bg-blue-50 hover:text-blue-600 cursor-pointer transition-colors flex items-center justify-between">
                                             <span>Finish</span>
                                             <i class="fas fa-check text-[10px] text-blue-500 opacity-0"></i>
                                         </div>
    ...
    window.selectMOStatus = (value, label) => {
        selectSearchableOption('mo_partial_status', value, label);
        const container = document.getElementById('mo_partial_status_options');
        if (container) {
            for (let opt of container.children) {
                const check = opt.querySelector('.fa-check');
                if (check) {
                    const attr = opt.getAttribute('onclick') || '';
                    const isMatch = attr.includes(`'${value}'`) || attr.includes(`"${value}"`);
                    check.classList.toggle('opacity-100', isMatch);
...
window.recalcPackingSacks = (idx) => {
    const row = document.querySelector(`.packing-result-row[data-tp-idx="${idx}"]`);
    if (!row) return;
    const inputVal = row.querySelector('.pack-input-actual')?.value || '0';
    const inputQty = parseFormattedNum(inputVal);
    const sacks = Math.floor(inputQty / 25);
    const sacksInput = row.querySelector('.pack-output-qty');
    if (sacksInput) sacksInput.value = sacks;
};
    ...
    if (mo.stage === 'OVEN_BASAH') {
        ...
        actualRows.forEach(row => {
            ...
            if (itemId && qtyActual > 0) {
                ...
                partialInputItems.push({ inventoryItemId: itemId, itemName, qty: qtyActual, unit: itemUnit });
            }
        });
        ...
        const packRows = document.querySelectorAll('.packing-result-row');
        const completedProducts = [];
        const outErrors = [];
        const rmErrors = [];
        ...
        packRows.forEach((row, idx) => {
            ...
            const inputQtyActual = parseFormattedNum(row.querySelector('.pack-input-actual')?.value);
            const outputSacks = parseFormattedNum(row.querySelector('.pack-output-qty')?.value);
            const inputWipId = row.getAttribute('data-wip-item-id');

            if (inputQtyActual <= 0) {
                outErrors.push(`${tp.itemName || `Baris ${idx+1}`}: Masukkan Qty bahan yang DI AMBIL/DIPAKAI`);
            }
            if (outputSacks < 0) {
                outErrors.push(`${tp.itemName || `Baris ${idx+1}`}: Jumlah sak tidak boleh negatif`);
            }

            // Validate stock in OVEN_KERING location
            if (inputWipId) {
                const srcLoc = 'OVEN_KERING';
                const stock = db.getInventoryStock(inputWipId, srcLoc);
                if (stock < inputQtyActual) {
                    rmErrors.push(`Stok WIP Oven Kering untuk ${tp.itemName || `Baris ${idx+1}`} tidak cukup (Sisa: ${prodFmt(stock)} Kg, Butuh: ${prodFmt(inputQtyActual)} Kg)`);
                }
            }

            totalInputActual += inputQtyActual;
            totalOutputSacks += outputSacks;

            completedProducts.push({
                ...tp,
                inputQtyActual,
                outputQtyActual: inputQtyActual, // primary output is Kg
                outputSacks
            });
        });
        ...
        updates.inputQty = totalInputActual;
        updates.outputQty = totalInputActual;
        updates.outputSacks = totalOutputSacks;
        if (completedProducts.length > 0) {
            updates.outputItemId = completedProducts[0].itemId;
            updates.inputItemId = completedProducts[0].wipItemId || db.ensureWIPItem(completedProducts[0].itemId, 'Oven Kering', false);
        }
        updates.targetProducts = completedProducts;
...
window.addMOTargetProductRow = (stageOrPresetId = '', presetName = '', presetId = '') => {
    ...
    div.innerHTML = `
            ...
            <div class="flex gap-1.5">
                <div class="relative flex-1 min-w-0">
                    ...
                    <div id="mo_product_dropdown_${rowId}"
                        ...
                        <div class="p-2 border-t border-slate-100 bg-slate-50/50">
                            <button type="button" onclick="openQuickAddProductModal('${rowId}')" class="w-full flex items-center gap-3 p-2.5 hover:bg-white hover:shadow-sm rounded-xl text-[11px] font-black text-slate-500 hover:text-indigo-600 transition-all uppercase tracking-wider">
                                <div class="w-7 h-7 rounded-lg bg-white border border-slate-200 flex items-center justify-center shadow-sm text-slate-400 group-hover:text-indigo-500"><i class="fas fa-plus"></i></div>
                                TAMBAH PRODUK BARU
                            </button>
                        </div>
                ...
                <button type="button" onclick="openQuickAddProductModal('${rowId}')"
                    class="shrink-0 w-8 h-8 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 hover:text-blue-700 transition-all flex items-center justify-center shadow-sm active:scale-95"
                    title="Tambah Produk Baru">
        ...
        <!-- Machine dropdown -->
        <div class="relative shrink-0${isPacking ? ' hidden' : ''}" id="${rowId}_wrap">
            <button type="button"
...
window.loadBOMMaterialsToMO = async () => {
    ...
    // Sync bomMaterials dari API kalau cache kosong
    const cachedMats = db.read('bomMaterials') || [];
    if (cachedMats.length === 0) {
        try { await db.sync('bomMaterials'); } catch(e) {}
    }
...
window.viewProductionMO = (id) => {
    ...
    // Calculate cumulative raw material consumed
    const cumulativeRM = {};
    (mo.history || []).forEach(h => {
        (h.inputItems || []).forEach(item => {
            const itemId = item.inventoryItemId || item.itemId;
            cumulativeRM[itemId] = (cumulativeRM[itemId] || 0) + parseFloat(item.qty || 0);
        });
    });

    let detailRows = '';
    if (mo.stage === 'OVEN_BASAH' && mo.inputItems) {
        detailRows = mo.inputItems.map(i => {
            const actualQty = cumulativeRM[i.inventoryItemId] || 0;
            return `
            <div class="flex justify-between items-center text-sm py-2 border-b border-slate-50 last:border-none">
                <span class="text-slate-700 font-semibold">${i.itemName}</span>
                <div class="text-right">
                    <span class="font-black text-slate-800">${prodFmt(actualQty)} ${i.unit}</span>
                    <span class="text-[10px] text-slate-400 block font-medium">Target: ${prodFmt(i.qty)} ${i.unit}</span>
                </div>
            </div>`;
        }).join('');
    } else if (mo.stage === 'FINISH_GOOD' && mo.fgItems) {
        detailRows = mo.fgItems.map(i => `
        <div class="flex justify-between items-center text-sm py-2 border-b border-slate-50 last:border-none">
            <span class="text-slate-700 font-semibold">${i.itemName}</span>
            <span class="font-black text-green-600">+${prodFmt(i.qty)} ${i.unit || 'Kg'}</span>
        </div>`).join('');
    }

    let historySection = '';
    if (mo.history && mo.history.length > 0) {
        const historyRows = mo.history.map((h, i) => {
            const dateStr = new Date(h.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' });
            const inpsHtml = (h.inputItems || []).map(itm => `<div class="text-[11px] font-semibold text-slate-700">${itm.itemName}: <span class="font-bold text-slate-900">${prodFmt(itm.qty)} ${itm.unit}</span></div>`).join('') || '<span class="text-slate-400">-</span>';
            const outsHtml = (h.outputProducts || []).map(op => `<div class="text-[11px] font-bold text-green-700">${op.itemName}: <span class="font-black text-green-900">+${prodFmt(op.qty)} Kg</span></div>`).join('') || '<span class="text-slate-400">-</span>';
            return `
                <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-none text-xs">
                    <td class="py-3.5 px-4 font-mono font-bold text-blue-600 whitespace-nowrap align-top">${dateStr}</td>
                    <td class="py-3.5 px-4 space-y-1 align-top">${inpsHtml}</td>
                    <td class="py-3.5 px-4 space-y-1 align-top">${outsHtml}</td>
                    <td class="py-3.5 px-4 font-medium text-slate-600 align-top">${h.createdBy || '-'}</td>
                    <td class="py-3.5 px-4 text-slate-500 italic align-top max-w-xs break-words" title="${h.notes || ''}">${h.notes || '-'}</td>
                </tr>
            `;
        }).join('');

        historySection = `
        <div class="space-y-3 mt-6 pt-6 border-t border-slate-100">
            <h4 class="text-[10px] font-black text-indigo-700 uppercase tracking-widest flex items-center gap-1.5 mb-2">
                <i class="fas fa-history"></i> Riwayat Penyelesaian Parsial
            </h4>
            <div class="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
                <table class="w-full text-left border-collapse min-w-[550px]">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th class="py-3 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Tanggal</th>
                            <th class="py-3 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Bahan Terpakai</th>
                            <th class="py-3 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Hasil Produksi</th>
                            <th class="py-3 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Operator</th>
                            <th class="py-3 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Catatan</th>
                        </tr>
                    </thead>
    ...
    const outputUnit = mo.stage === 'OVEN_BASAH' ? 'Kg' : 'Krg';

    const body = `
    <div class="space-y-6">
        <!-- Section 1: Header Info Card Grid -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 bg-slate-50 p-6 rounded-3xl border border-slate-100">
            <div>
                <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Nomor MO</span>
                <span class="text-sm font-mono font-bold text-slate-800">${mo.moNumber}</span>
            </div>
            <div>
                <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Status MO</span>
                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider inline-block ${prodStatusColor(mo.status)}">${mo.status === 'DONE' ? 'Selesai' : (mo.status === 'PARTIAL' ? 'Sebagian' : 'Proses')}</span>
            </div>
            <div>
                <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Tahap Produksi</span>
                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider inline-block ${prodStageColor(mo.stage)}">${prodStageLabel(mo.stage)}</span>
            </div>
            <div class="sm:col-span-2 lg:col-span-3">
                <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Produk Jadi</span>
                <span class="text-sm font-black text-slate-850">${mo.productName}</span>
            </div>
            <div>
                <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Tanggal Mulai</span>
                <span class="text-sm font-bold text-slate-700">${prodDate(mo.date)}</span>
            </div>
            <div>
                <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Tanggal Selesai</span>
                <span class="text-sm font-bold text-slate-700">${prodDate(mo.completedAt) || '-'}</span>
            </div>
            <div>
                <span class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Total Input Awal</span>
                <span class="text-sm font-black text-slate-800">${prodFmt(mo.inputQty)} Kg</span>
            </div>
        </div>

        <!-- Section 2: Side-by-Side Actual Metrics -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Left Card: Bahan Baku -->
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 border-b border-slate-100 pb-3">
                    <i class="fas fa-cubes text-blue-600"></i> Bahan Baku (Realisasi / Target)
                </h4>
                <div class="space-y-1">
                    ${detailRows || '<div class="text-sm text-slate-400 italic">Tidak ada bahan baku</div>'}
                </div>
            </div>

            <!-- Right Card: Output Produk -->
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
                <h4 class="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 border-b border-slate-100 pb-3">
                    <i class="fas fa-box-open text-emerald-600"></i> Total Output Realisasi
                </h4>
                <div class="space-y-1">
                    ${(mo.status === 'DONE' || mo.status === 'PARTIAL')
                    ? (mo.outputProducts && mo.outputProducts.length > 0
                        ? `<div class="space-y-2">${mo.outputProducts.map(op => `
                            <div class="flex justify-between items-center text-sm py-2 border-b border-slate-50 last:border-none">
                                <span class="font-semibold text-slate-700">${op.itemName}</span>
                                <span class="font-black text-emerald-600">+${prodFmt(op.qty)} ${outputUnit}</span>
                            </div>`).join('')}</div>`
                        : `<div class="flex justify-between items-center text-sm py-2">
                            <span class="font-semibold text-slate-700">${mo.productName}</span>
                            <span class="font-black text-emerald-600">+${prodFmt(mo.outputQty)} ${outputUnit}</span>
                           </div>`)
                    : '<div class="text-sm text-slate-400 italic py-2">Belum ada output yang selesai</div>'}
                </div>
                ${mo.shrinkagePct > 0 ? `
                <div class="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center text-xs">
                    <span class="font-semibold text-orange-600 uppercase tracking-wider">Persentase Penyusutan</span>
                    <span class="font-black text-orange-600 bg-orange-50 px-2.5 py-1 rounded-lg">${mo.shrinkagePct}%</span>
                </div>` : ''}
            </div>
        </div>
        ...
        ${mo.notes ? `
        <div class="bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50 text-xs text-blue-700 whitespace-pre-wrap flex items-start gap-2.5">
            <i class="fas fa-info-circle text-sm mt-0.5"></i>
            <div><strong>Catatan MO:</strong><br>${mo.notes}</div>
        </div>` : ''}

        <!-- Section 4: History Table -->
        ${historySection}
    </div>`;
    showModal(`Detail MO - ${mo.moNumber}`, body, `<button onclick="closeModal()" class="btn-secondary">Tutup</button>`, 'lg');
};

window.viewMOHistoryModal = (id) => {
    const mo = db.findById('productionOrders', id);
    if (!mo) return;
    const history = mo.history || [];
    if (history.length === 0) {
        showToast('Tidak ada riwayat penyelesaian parsial pada MO ini.', 'warning');
        return;
    }
    ...
    const bodyHtml = `
        <div class="space-y-4">
            <div class="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100/50 text-xs text-indigo-700 font-medium">
                Riwayat penyelesaian parsial untuk MO: <strong class="font-mono text-indigo-900">${mo.moNumber}</strong> (${mo.productName || ''})
            </div>
    ...
    showModal(`Riwayat Parsial - ${mo.moNumber}`, bodyHtml, `<button onclick="closeModal()" class="btn-secondary">Tutup</button>`);
};
...
window.updateProductionReports = () => {
    ...
    const mos = (db.read('productionOrders') || []).filter(mo => {
        if (mo.status !== 'DONE' && mo.status !== 'PARTIAL') return false;
...
// ─── QUICK ADD PRODUCT MODAL IN MO FORM ────────────────────────────────
window.openQuickAddProductModal = (rowId) => {
    const stage = document.getElementById('mo_stage')?.value;
    let defaultCat = 'FINISHED_GOODS';
    if (stage === 'OVEN_BASAH') {
        defaultCat = 'OVEN_BASAH_STOCK';
    } else if (stage === 'OVEN_KERING') {
        defaultCat = 'OVEN_KERING_STOCK';
    ...
    const allCats = [
        ...
        ['OVEN_BASAH_STOCK', 'Oven Basah'], 
        ['OVEN_KERING_STOCK', 'Oven Kering'],
        ['RAW_MATERIAL', 'Bahan Baku & Bumbu'], 
        ['PACKAGING', 'Kemasan']
    ];
    const catOpts = allCats.map(([val, label]) => `<option value="${val}" ${val === defaultCat ? 'selected' : ''}>${label}</option>`).join('');

    const nextCode = db.generateItemCode(defaultCat);

    const bodyHtml = `
        <div class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Kategori Barang <span class="text-red-500">*</span></label>
                    <select id="quick_p_category" onchange="onQuickCategoryChange(this.value)" class="w-full border-2 border-slate-200 rounded-xl px-4 py-3 bg-white font-bold text-slate-800 focus:border-indigo-500 outline-none transition-all">
                        ${catOpts}
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Kode Barang <span class="text-slate-300">(Auto)</span></label>
                    <input type="text" id="quick_p_code" value="${nextCode}" class="w-full border-2 border-slate-200 rounded-xl px-4 py-3 bg-slate-50 font-bold text-slate-800 outline-none" readonly>
                </div>
            </div>
            <div>
                <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Nama Barang <span class="text-red-500">*</span></label>
                <input type="text" id="quick_p_name" placeholder="Ketik nama produk..." class="w-full border-2 border-slate-200 rounded-xl px-4 py-3 bg-white font-bold text-slate-800 focus:border-indigo-500 outline-none transition-all">
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Satuan <span class="text-red-500">*</span></label>
                    <select id="quick_p_unit" class="w-full border-2 border-slate-200 rounded-xl px-4 py-3 bg-white font-bold text-slate-800 focus:border-indigo-500 outline-none transition-all">
                        ${unitOpts}
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Stok Minimum</label>
                    <input type="number" id="quick_p_minstock" value="0" min="0" class="w-full border-2 border-slate-200 rounded-xl px-4 py-3 bg-white font-bold text-slate-800 focus:border-indigo-500 outline-none transition-all">
                </div>
            </div>
        </div>
    `;
    ...
    const footerHtml = `
        <div class="flex items-center justify-end gap-3 w-full">
            <button type="button" onclick="closeModal()" class="px-6 py-2.5 bg-slate-100 text-slate-500 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95">Batal</button>
            <button type="button" onclick="saveQuickAddProduct('${rowId}')" id="quick_p_save_btn" class="px-8 py-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg active:scale-95 flex items-center gap-2">
                <i class="fas fa-check-circle text-[10px]"></i> Simpan & Pilih
            </button>
        </div>
    `;

    window.onQuickCategoryChange = (cat) => {
        const input = document.getElementById('quick_p_code');
        if (input) {
            input.value = db.generateItemCode(cat);
        }
    };

    showModal('Tambah Produk Baru Secara Cepat', bodyHtml, footerHtml, 'md');
};

window.saveQuickAddProduct = async (rowId) => {
    const name = document.getElementById('quick_p_name')?.value.trim();
    const category = document.getElementById('quick_p_category')?.value;
    const unit = document.getElementById('quick_p_unit')?.value;
    const minStock = parseFloat(document.getElementById('quick_p_minstock')?.value) || 0;
    ...
    const saveBtn = document.getElementById('quick_p_save_btn');
    if (!saveBtn) return;
    ...
    try {
        ...
        const newItem = items.find(it => it.itemName === name && it.category === category) || result;
        
        if (newItem && newItem.id) {
            selectMOProduct(rowId, newItem.id, newItem.itemName);
        }
